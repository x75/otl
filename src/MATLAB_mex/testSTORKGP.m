...
sgph = createSTORKGP( input_dim, output_dim, tau, ...
    [l, rho, alpha], noise, epsilon, capacity);

for i=1:N-future_step
    %update with the input (note we have to provide the handle)
    updateSTORKGP(sgph, x_data(i,:));
    
    %get the prediction
    [pred_mean(i,:), pred_var(i,:)] = predictSTORKGP(sgph);
    
    %compute the error (with the TRUE y data)
    error(i) = norm(pred_mean(i,:) - y_data(i,:));
    
    %train the model with the observation
    trainSTORKGP(sgph, y_data_obs(i,:));
