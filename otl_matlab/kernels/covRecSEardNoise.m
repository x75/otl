function K = covRecSEardNoise(hyp, x, z, hd)
...
if nargin<2, K =sprintf('(%d+3)', rD_recKern_7389); return; end              % report number of parameters
...
sf2 = exp(2*hyp(D+1));
rho = hyp(D+2);% 1/(1+exp(-ori_rho));
s2 = exp(2*hyp(D+3)); %noise parameter
...
if nargin>3                                                        % derivatives
    if hd<=D                                              % length scale parameters
        if dg
            ...
            if xeqz
                K = getRecKernMat(ell, tau, rho, hd, x');
            else
                K = getRecKernMat(ell, tau, rho, hd, x', z');
            end
            
            K = sf2*K;
            
        end
    elseif hd==D+1                                            % magnitude parameter
        if dg
            CRK_tau = ones(D,1);
        else
            if xeqz
